...
public:
	...
	bool is_same_key(const DoubleArrayKey &target)
	{
...
private:
	...
	static const char CharSizeCheck[
		sizeof(CharType) == sizeof(UCharType) && sizeof(ValueType)];
...
public:
	...
	void set_result(value_type *result, value_type value, size_t) const
	{
		*result = value;
	}
	void set_result(result_pair_type *result_pair,
		value_type value, size_t length) const
	{
		result_pair->value = value;
		result_pair->length = length;
	}

	DoubleArrayImpl() : unit_(0), tail_(0), unit_vector_(), tail_vector_() {}
...
private:
		...
		ProgressFunc progress_func)
	{
		...
		for (size_t i = 0; i < num_of_keys; ++i)
		{
			...
			if (keys_list.empty() || !key.is_same_key(keys_list.back()))
...
typedef DoubleArrayImpl<
	char, unsigned char, int, unsigned int> DoubleArray;
typedef DoubleArrayImpl<
	char, unsigned char, int, unsigned long long> HugeDoubleArray;
